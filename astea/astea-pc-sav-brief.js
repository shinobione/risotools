...
const code=await fetchCode('astea/astea-pc-interventions-v2.js');
eval(code);
let ov=null,ta=null;for(let i=0;i<650;i++){ov=document.getElementById('shinoAsteaPcOut');ta=ov?.querySelector('textarea')||null;if(ta)break;await S(100)}
if(!ov||!ta)throw Error('Résultat PC introuvable après le scan.');
const buttons=[...ov.querySelectorAll('button')],close=buttons.find(b=>/FERMER/i.test(b.textContent||'')),bar=close?.parentElement;
if(!bar)throw Error('Barre d’actions PC introuvable.');
if(document.getElementById('shinoAsteaSavPc'))return;
const sav=document.createElement('button');sav.id='shinoAsteaSavPc';sav.textContent='🧰 COPIER POUR CHATGPT — PRÉPARER TOURNÉE';sav.style='min-height:50px;font-weight:800;border-radius:10px;border:0;grid-column:1 / -1';
...
}catch(e){alert('ShinoAstea SAV PC : '+e.message)}})();
